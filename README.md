...
```mermaid
graph TD
    Main[DNAAutomatorEngine 主引擎] --> SM[ServiceManager 服务管理器]
    Main --> CR[CommandRouter 命令路由器]
    Main --> CM[ConfigManager 配置管理器]
    
    SM --> WS[WindowService 窗口服务]
    SM --> SS[ScriptService 脚本服务]
    SM --> IS[ImageService 图像识别服务]
    
    CR --> WH[WindowCommandHandler]
    CR --> SH[ScriptCommandHandler]
    CR --> SYH[SystemCommandHandler]
    
    WS --> WC[WindowCapture 窗口捕获]
    SS --> HM[HumanMouse 鼠标控制]
    IS --> IR[ImageRecognition 图像识别]
...
- [x] 实时日志显示系统
- [x] 基础 UI 界面(使用 Element Plus)
- [x] 游戏窗口检测功能
- [x] 窗口连接和捕获功能
- [x] **Python后端架构重构** - 基于SOLID原则的服务化架构
- [x] **模块化前端重构** - Hook + 组件分离架构
- [x] **跨平台支持** - Windows/macOS/Linux三平台兼容
- [x] **图像识别系统** - 完整的图像识别和自动点击功能
- [x] **管理员权限管理** - 跨平台权限检测和管理
- [x] **配置管理系统** - 统一的配置文件管理
- [x] **调试工具集** - 完整的调试和测试工具
...
### Issue #25: fire10脚本重构为角色材料副本 - 实例化架构设计
**需求**:
- 将fire10脚本重构为更通用的角色材料副本脚本
- 使用实例化设计模式，提高代码的可维护性和扩展性
- 增加更多配置选项，支持多种副本类型和材料类型

**实现方案**:

**1. 脚本标识符重构**:
- 将 `fire10` 重构为 `material-dungeon`
- 更新所有相关的配置和组件引用
- 保持向后兼容性，平滑迁移用户配置
...
**3. 配置选项设计**:
- **副本类型**: 选择要刷取的副本类型，决定脚本的开始和结束逻辑
- **每次副本轮次**: 每次进入副本刷取的轮次数（1-99轮），一般设置10轮撤离
- **超时时间**: 防止脚本卡死的保护机制（60-3600秒）
- **刷本顺序**: 用户可自定义刷本顺序，支持6种属性材料（光、暗、水、电、火、风）
- **循环执行**: 按配置顺序循环刷取，如"光1暗1"表示刷1次光再刷1次暗然后重复
- **智能管理**: 自动计算每个循环的总副本次数

**4. 用户界面升级**:
- 使用Element Plus组件库构建现代化界面
- 添加图标和视觉提示，提升用户体验
- 提供详细的选项描述和使用建议
- 实现响应式设计，适配不同屏幕尺寸

**用户体验改进**:
- ✅ **配置更灵活**: 支持多种副本类型和材料类型选择
- ✅ **界面更友好**: 清晰的选项描述和智能提示
- ✅ **功能更完善**: 体力管理、时间预估等实用功能
- ✅ **扩展性更强**: 易于添加新的脚本类型和配置选项
...
**使用方法**:
1. 在脚本选择面板中选择"角色材料副本"
2. 选择副本类型（目前支持"角色材料副本"）
3. 设置每次副本轮次（推荐10轮）和超时时间
4. 配置刷本顺序：选择材料属性和刷取次数
5. 可添加多个刷本配置，脚本将按顺序循环执行
6. 点击"脚本，启动！"开始自动化执行

**配置示例**:
- 火1：只刷火材料副本，每次10轮，循环执行（默认配置）
- 火2水1：刷2次火材料副本 → 刷1次水材料副本 → 重复
- 光1暗1电1：刷1次光材料副本 → 刷1次暗材料副本 → 刷1次电材料副本 → 重复
...
### Issue #27: role-dungeon模块交互优化 - 多选下拉框重构
**需求**:
- 使用多选下拉框替代添加/删除按钮，提供更友好的用户体验
- 用户可以直接在下拉框中选择/取消材料，自动管理配置列表
- 移除复杂的添加和删除按钮，简化界面操作
...
```vue
<!-- 新的多选下拉框交互 -->
<el-select v-model="selectedMaterials" multiple>
  <el-option v-for="option in materialOptions" :value="option.value">
    <!-- 材料图标 + 名称 -->
  </el-option>
</el-select>
```
...
**2. 自动配置管理**:
- **选择材料**: 在下拉框中勾选材料，自动添加到配置列表
- **取消材料**: 在下拉框中取消勾选，自动从配置列表移除
- **智能排序**: 配置列表按照材料选项的原始顺序自动排序
- **默认值**: 新添加的材料自动设置为1次、10轮的默认配置

**3. 界面优化**:
- **移除按钮**: 删除了"添加刷本配置"和"删除"按钮
- **清晰布局**: 每个配置项显示材料图标、名称和两个数值配置
- **空状态提示**: 未选择材料时显示友好的提示信息
- **响应式设计**: 配置项支持换行，适配不同屏幕尺寸
...
**用户体验改进**:
- ✅ **操作更直观**: 直接在下拉框中选择，无需点击添加按钮
- ✅ **管理更简单**: 取消勾选即可删除，无需找删除按钮
- ✅ **界面更简洁**: 移除了复杂的按钮操作，界面更清爽
- ✅ **反馈更及时**: 选择变化立即反映在配置列表中

**技术实现亮点**:
- 双向数据绑定：多选框状态与配置列表完全同步
- 智能差异检测：精确识别新增和删除的材料
- 自动排序机制：保持配置列表的一致性顺序
- 响应式更新：配置变化实时同步到父组件
...
### Issue #26: role-dungeon模块配置优化 - 副本类型和参数重构
**需求**:
- 为role-dungeon模块添加副本类型配置，用于确定开始和结束脚本
- 修改默认配置，去掉火水循环，只保留火循环
- 新增dungeonSequence参数，配置每次打多少轮次（默认10）
- 去掉原来的maxRounds参数，使用新的dungeonSequence参数
...
interface MaterialDungeonConfig {
  ...
  dungeonSequence: number     // 每次打多少轮次(1-99，默认10)
...
const defaultConfig: MaterialDungeonConfig = {
  ...
  dungeonSequence: 10,          // 每次打10轮次
  timeout: 300,                 // 5分钟超时
  materialSequence: [
    { material: '火', times: 1 } // 默认只刷火循环
  ]
}
```

**3. 副本类型选项**:
- **角色材料副本**: 角色升级材料副本，包含光、暗、水、电、火、风六种属性材料
- 未来可扩展：武器材料副本、经验副本等

**4. 用户界面改进**:
- 新增副本类型选择器，带有详细描述
- 重命名"每次副本轮次"参数，更清晰的说明
- 优化材料序列配置界面
- 更新使用示例和说明文档

**用户体验改进**:
- ✅ **配置更清晰**: 副本类型明确，用户知道脚本会如何执行
- ✅ **默认更合理**: 默认只刷火循环，符合大多数用户需求
- ✅ **参数更直观**: dungeonSequence参数名称更直观易懂
- ✅ **扩展性更强**: 副本类型配置为未来扩展奠定基础

**技术改进**:
- 参数重命名：maxRounds → dungeonSequence
- 配置重构：dungeonSequence → materialSequence
- 新增副本类型配置和选择器
- 更新所有相关的函数和变量名
...
**5. 智能校验机制**:
- **防重复配置**: 每种材料属性最多只能配置一个，选择器会自动禁用已选择的材料
- **动态添加限制**: 当所有6种材料都配置完后，"添加刷本配置"按钮自动禁用
- **智能默认值**: 添加新配置时自动选择第一个未配置的材料属性
- **实时验证**: 配置变更时实时更新描述和统计信息

**6. 用户界面优化**:
- **双列配置**: 每行显示"刷取次数"和"副本轮次"两个配置项
- **详细描述**: 循环顺序描述显示完整信息，如"火1次(10轮) → 水2次(15轮)"
- **状态提示**: 当无法添加更多配置时显示友好提示
- **图标预览**: 每个配置项显示对应材料的图标预览

**向后兼容性**:
- 保持组件ID不变（material-dungeon）
- 自动迁移旧配置到新格式
- 保持核心功能逻辑不变

---

### Issue #28: role-dungeon模块交互增强 - 防删除保护和拖拽排序
**需求**:
- 防止用户删除最后一个材料配置，至少保留一个
- 添加上下移动按钮，让用户可以自定义材料顺序
- 增加流畅的动画效果，提升用户体验
...
**2. 上下移动功能**:
- **向上移动**: `moveItemUp(index)` - 将当前项向上移动一位
- **向下移动**: `moveItemDown(index)` - 将当前项向下移动一位
- **智能禁用**: 第一项禁用向上按钮，最后一项禁用向下按钮
- **同步更新**: 移动后自动同步多选下拉框的选中状态
...
**4. 用户界面优化**:
- **移动按钮**: 右侧显示上下箭头按钮，仅在多个配置时显示
- **按钮状态**: 智能禁用不可用的移动方向
- **视觉反馈**: 按钮悬停时放大和变色效果
- **动画延迟**: 150ms延迟执行移动，让用户看到动画效果

**用户体验改进**:
- ✅ **防误操作**: 无法删除最后一个材料，避免配置为空
- ✅ **友好提示**: 删除失败时显示清晰的警告消息
- ✅ **自由排序**: 可以随意调整材料的执行顺序
- ✅ **流畅动画**: 移动操作有平滑的视觉反馈
- ✅ **智能交互**: 按钮状态根据位置智能启用/禁用

**技术实现亮点**:
- 状态保护：防止无效操作并恢复之前状态
- 动画控制：通过DOM操作和CSS类实现流畅动画
- 双向同步：移动操作同时更新配置数组和选择状态
- 用户体验：延迟执行让动画效果更明显

**操作流程**:
1. **正常删除**: 在多选框中取消勾选材料，正常删除配置
2. **保护机制**: 尝试删除最后一个材料时，显示警告并阻止操作
3. **调整顺序**: 点击上下箭头按钮调整材料执行顺序
4. **动画反馈**: 移动时显示平滑的动画效果
...
- [x] **窗口捕获调试**: 创建了专门的窗口捕获调试组件，用于调试游戏窗口连接
...
#### 前端组件
- `WindowCaptureDebug.vue`: 游戏窗口捕获调试组件
...
图像模板文件位于 `static/role-dungeon/` 目录：
- `火.png` - 火副本图标
- `水.png` - 水副本图标  
- `风.png` - 风副本图标
- `电.png` - 电副本图标
...
## 🔍 游戏窗口捕获调试 (Window Capture Debug)

### 功能说明
专门用于调试游戏窗口的检测、连接和捕获功能的组件，帮助用户快速定位窗口连接问题。

### 主要功能
1. **连接状态显示**: 实时显示当前游戏窗口的连接状态
2. **窗口捕获测试**: 测试窗口检测和连接功能是否正常
3. **窗口置顶测试**: 验证窗口激活和置顶功能
4. **实时调试日志**: 显示详细的调试信息和错误日志

### 使用方法
1. 启动应用后，在主界面找到"游戏窗口捕获调试"面板
2. 点击"🔍 测试窗口捕获"按钮开始测试
3. 如果已连接窗口，可以点击"🔝 测试窗口置顶"验证置顶功能
4. 查看调试日志了解详细的执行过程和可能的错误
...
## 🧹 项目清理完成 (Cleanup Completed)

### ✅ 已清理的测试代码
- [x] **前端测试组件**: 删除了 `TestMouseKeyboard.vue` (1570行测试代码)
- [x] **Python调试文件**: 删除了所有debug_*.py、test_*.py等调试文件
- [x] **测试文档**: 删除了测试说明、调试指南等文档
- [x] **调试数据**: 删除了所有coordinate_debug_*.json等调试数据文件
- [x] **Python主文件**: 重构了main.py，移除了所有测试命令处理逻辑
- [x] **缓存文件**: 清理了Python __pycache__ 目录

### 🏗️ Python后端架构重构 (Backend Architecture Refactoring)

#### ✅ 新架构特点
- [x] **服务化架构**: 采用微服务设计，每个功能模块都是独立的服务
- [x] **SOLID原则**: 遵循单一职责、开闭原则等面向对象设计原则
- [x] **命令路由**: 统一的命令处理和路由机制，支持命令验证和错误处理
- [x] **依赖注入**: 服务之间通过依赖注入解耦，便于测试和扩展
- [x] **生命周期管理**: 统一的服务生命周期管理（初始化、启动、停止）

#### 📁 新的目录结构
```
py_engine/
├── core/                    # 核心模块
│   ├── base_service.py     # 服务基类和服务管理器
│   ├── command_handler.py  # 命令处理器和路由器
│   └── __init__.py
├── services/               # 业务服务
│   ├── window_service.py   # 窗口服务（检测、连接、激活）
│   ├── script_service.py   # 脚本服务（执行、管理、统计）
│   └── __init__.py
├── main.py                 # 主入口文件（重构版）
├── window_capture.py       # 窗口捕获模块（保留）
├── image_recognition.py    # 图像识别模块（保留）
├── human_mouse.py         # 鼠标控制模块（保留）
└── requirements.txt
```

#### 🔧 架构优势
1. **模块化**: 每个服务职责单一，易于理解和维护
2. **可扩展**: 新增功能只需创建新服务，无需修改现有代码
3. **可测试**: 服务之间解耦，便于单元测试
4. **错误处理**: 统一的错误处理和日志记录机制
5. **配置管理**: 集中的配置管理和验证

### 🆕 新增功能
- [x] **窗口捕获调试组件**: 创建了专门的 `WindowCaptureDebug.vue` 组件
- [x] **服务化Python引擎**: 完全重构的后端架构，遵循最佳实践
- [x] **命令路由系统**: 统一的命令处理和验证机制
- [x] **清晰的项目结构**: 项目现在更加简洁，专注于核心功能
